...
class FilesRepository {
    ...
    async getById(id: string) {
        ...
        const file = await MongoFile.findById(id)
			.select('originalName status mimeType createdAt size processedData.summary'); // TODO: use select only the fields we need
    ...
    async findAll(query: {
        ...
        const files = await MongoFile.find(query)
            ...
            .skip(skip)
            .limit(limit)
			.select('originalName status mimeType createdAt size processedData.summary');
